...
const EFVector = (pos, point_charge) => {
    ...
    const r_sq =
        diff.lengthSq();    // 点電荷と観測点との距離^2 (平方根処理をなくすため)
...
class ElectricLines3D extends THREE.Object3D
{
    ...
    createElectricLineGeometry =
        (origin_charge, vector, point_charges, length) => {
                ...
                for (let n_phi = 0; n_phi < 10; n_phi++)
                {
                    ...
                    const line = new THREE.Line(
                        this.createElectricLineGeometry(
                            point_charge, new THREE.Vector3(x, y, z), point_charges,
                            1000),
                        line_material);
                    this.add(line);
                }
    ...
    initEvents = () => {
        const onPointerMove = (event) => {
            ...
            const intersects = this.raycaster.intersectObjects(
                this.point_charges_3d.children, false);
            ...
            console.log(this.point_charges);
        };

        this.dom.addEventListener('mousemove', onPointerMove);
    };
